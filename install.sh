...
then
  ...
  apt-get install tree mc isc-dhcp-server openssh-server apache2 htop ufw unattended-upgrades apt-get install cabextract libmspack0  rkhunter ntp samba smbclient winbind krb5.config krb5.user -y
